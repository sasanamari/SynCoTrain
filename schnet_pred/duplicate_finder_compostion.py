...
synthDF['composition'] = synthDF['structure'].apply(lambda x: x.composition.reduced_formula)
...
def find_duplicates_by_composition(df1, df2, matcher):
    ...
    for i, comp in enumerate(df1_grouped.groups.keys()):
        ...
        if (i + 1) % 1000 == 0:
            ...
            print(f"Processed {i + 1} compositions. Estimated time left: {format_time(estimated_time_left)}", flush=True)
...
duplicates_by_composition_df = pd.DataFrame(duplicates_by_composition, columns=['oqmd_material_id', 'synthDF_material_id'])
duplicates_by_composition_df.to_csv('schnet_pred/data/duplicate_materials_by_composition.csv', index=False)
