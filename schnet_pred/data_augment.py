...
def get_gmm():
    with open(base_dir / "dists.pkl", "rb") as file:
        dist = pkl.load(file)
...
if __name__ == "__main__":
    ...
    # Load your data
    propDFpath = base_dir / '..' / source
    # df = pd.read_pickle(propDFpath)
    print('Reading data...')
    if symmetrical_test:
        train_val_df = split_and_save_symmetrical_test(propDFpath= propDFpath, test_save_path= data_dir/test_filename)
    else:        
        train_val_df = split_and_save_test(propDFpath= propDFpath, test_save_path= data_dir/test_filename)
