...
# %%
labeldf = pd.merge(coAldf[['material_id', 'avg_prediction', prop]],
            coSchdf[['material_id', 'predScore', prop]], on = "material_id", how = "outer")
...
labeldf[f"{prop}_preds"]=labeldf[f"{prop}_avg"].map(lambda x: 0 if x< clssification_cutoff else 1)
