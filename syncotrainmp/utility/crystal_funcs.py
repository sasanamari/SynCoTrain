...
def exper_oxygen_query(
    MPID: str,
    theoretical_data=False,
    num_sites=(1, 150),
    fields: Union[str, List[str]] = "default",
) -> str:
    """
...
def oxide_check(initStruc: Structure) -> Tuple[bool, bool, bool, Structure]:
    """
    Checks the oxidation states of the atoms in primary cell. Returns the primary structure, and
    ...
    try:
        ...
        return (
            ...
            bad_structure,
            primStruc,
        )  # function quits for bad structures
    ...
    for i, site in enumerate(
        primStruc
    ):  # checks for any element other than Oxygen with a negative oxidation state.
...
def clean_oxide(
    experimental: bool,
    pymatgenArray: np.ndarray,
    reportBadData: bool = False,
    read_oxide_type: bool = True,
) -> np.ndarray:
    """
    ...
    for j, material in enumerate(pymatgenArray):
        ...
        try:
            ...
            if other_anion:
                ...
                material["structure"] = (
                    primStruc  # replaces the existing structure with the primitive structure
                )
    ...
    goodata = np.delete(
        pymatgenArray,
        [*anion_ind, *unstable_ind, *oxid_ind, *valence_ind, *structure_ind],
    )
    # del pymatgenArray
