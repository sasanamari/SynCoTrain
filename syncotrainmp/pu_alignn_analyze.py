...
def load_experiment_results(output_dir, max_iter):
    ...
    resdf["material_id"] = resdf["id"].map(
        lambda material_id: re.split("CAR-|.vas", material_id)[1]
    )
...
def compute_aggregate_df(resdf, propDF, prop, pseudo_label_threshold):
    ...
    aggdf["new_labels"] = aggdf["avg_prediction"].map(
        lambda x: 1 if x >= pseudo_label_threshold else 0
    )
...
def compute_cotrain_labels(propDF, aggdf, TARGET, prop):
    ...
    if cotrain_df["new_labels"].isna().mean() > 0.02:
        warnings.warn(
            f"{round(cotrain_df['new_labels'].isna().mean(), 3) * 100}% of 'new_labels' are NaN.",
            RuntimeWarning,
        )
    else:
        print(
            f"Rate of NaN values in predicted labels is: {cotrain_df['new_labels'].isna().mean()}"
        )
...
def pu_report_alignn(
    ...
    output_dir = os.path.join(
        output_dir, alignn_dir, f"PUOutput_{data_prefix}{experiment}"
    )
