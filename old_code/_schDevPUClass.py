...
for it in range(start_iter, num_iter):
    ...
    best_model = best_model.to('cpu')   #we need this because our it_test_df lives on the cpu
    # we could avoid this if we managed to use trainer.predict instead.
    
    it_test_df['pred_'+str(it)] = it_test_df.atoms.apply(lambda x: pred_test(x, best_model=best_model))
    it_test_df['pred_score'+str(it)] = it_test_df.atoms.apply(lambda x: pred_test(x, best_model=best_model, returnScore=True))
