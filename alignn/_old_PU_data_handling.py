"""Modified alignn.data for producing balanced datasets for PU-classification."""
...
def get_id_train_val_test_PU(
    ...
    print('the size of positive data is ', data_size["positive_data_size"])
    if (
        train_ratio is None
    ...
    if n_train is None:
        n_train = int(train_ratio * positive_size)
        # n_train = int(train_ratio * total_size)
        # data_size["positive_data_size"]
    if n_val is None:
        n_val = int(val_ratio * positive_size)
    if n_test_positive is None:
        n_test_positive = int(test_ratio * positive_size)
        n_test_unlabeled = int(unlabeled_size - (n_train + n_val) )
    ids_positive = list(np.arange(positive_size))
    ids_unlabeled = list(np.arange(positive_size, positive_size + unlabeled_size))
    if not keep_data_order:
        random.seed(split_seed)
        random.shuffle(ids_positive)
        random.shuffle(ids_unlabeled)
    # np.random.shuffle(ids)
    if n_train + n_val + n_test_positive > positive_size:
        raise ValueError(
            "Check total number of samples.",
            n_train + n_val + n_test_positive,
            ">",
            positive_size,
        )
    ...
    # positive data
    id_train_positive = ids_positive[:n_train]
    ...
    # unlabeled ids 
    id_train_unlabeled = ids_unlabeled[:n_train]
    id_val_unlabeled = ids_unlabeled[-(n_val + n_test_unlabeled) : -n_test_unlabeled]  
    id_test_unlabeled = ids_unlabeled[-n_test_unlabeled:]
    
    # all data
    id_train = id_train_positive + id_train_unlabeled
    id_val = id_val_positive + id_val_unlabeled
    id_test = id_test_positive + id_test_unlabeled
...
def get_train_val_loaders_PU(
    ...
    if (
        ...
        all_targets = []  #only used for max/min/avg, order not important.
        ...
        id_train, id_val, id_test = get_id_train_val_test_PU(
            # total_size=len(dat),
            positive_size=data_size['positive_data_size'],
            unlabeled_size=data_size['unlabeled_data_size'],
        ...
        # here we shuffle positive and unlabeled data for train, val and test sets.
        random.Random(split_seed).shuffle(dataset_train)
        random.Random(split_seed+1).shuffle(dataset_val)
        random.Random(split_seed+2).shuffle(dataset_test)
        # The memory overflow happens after the above shuffling.
