...
def label_dist3(codf, datadf, ehull=False,prop = 'synth', filename=None):
    ...
    plt.show()
# %%
def label_dist4(codf, datadf, pred_col = 'prediction' ,ehull=False,prop = 'synth', filename=None):
    plot_df = codf.merge(datadf[["material_id", "formation_energy_per_atom", "energy_above_hull"]], on="material_id")
    if ehull:
        prop = "stability"     
    edf = plot_df[plot_df[prop] == 1]
...
# %%
# %%
def midlabel_dist(codf, datadf, ehull=False,prop = 'synth',figtitle = None, filename=None):
    ...
    max_height = n.max()
    ax1.set_title('Synthesizability Score for Experimental Data', fontsize=13.5, fontweight='bold')
    ...
    ax2.arrow(.5,max_height*.7,.35,0, linewidth=3, color ='r', length_includes_head=True,
            head_width=max_height*.08, head_length=0.05)
    ax2.set_title('Synthesizability Score for Theoretical Data', fontsize=13.5,fontweight='bold')
...
# %%
def final_labels(plot_df, ehull=False,prop = 'synth',figtitle = None, filename=None,
                 threshold = 0.5):
    edf = plot_df[plot_df[prop] == 1]
    tdf = plot_df[plot_df[prop] == 0]
    # prob = "prediction"
    ...
    max_height = n.max()
    ax1.set_title('Synthesizability Prediction for Experimental Data', fontsize=13.5, fontweight='bold')
    ...
    ax2.arrow(threshold,max_height*.7,.35,0, linewidth=3, color ='r', length_includes_head=True,
            head_width=max_height*.08, head_length=0.05)
    ax2.set_title('Synthesizability Prediction for Theoretical Data', fontsize=13.5,fontweight='bold')
